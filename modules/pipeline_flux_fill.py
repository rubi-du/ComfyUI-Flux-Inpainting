...
def calculate_shift(
    ...
    """ Calculates shift based on sequence length """
...
def retrieve_latents(
    ...
    """ Retrieves the latents from an encoder output """
...
class FluxFillPipeline(
    ...
    def __init__(
        ...
        """ Initialize a new instance of the flux fill pipeline """
        super().__init__()
        ...
        if prompt_embeds is not None and pooled_prompt_embeds is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `pooled_prompt_embeds` also have to be passed.\
                    Make sure to generate `pooled_prompt_embeds` from the same text encoder that\
                        was used to generate `prompt_embeds`."
            )
    ...
    def _prepare_latent_image_ids(batch_size, height, width, device, dtype):
        """ Generate an array containing the ids of each pixel in the latent space """
    ...
    def _pack_latents(latents, batch_size, num_channels_latents, height, width):
        """ Pack the given latents so they can be processed as a single vector """
    ...
    def _unpack_latents(latents, height, width, vae_scale_factor):
        """ Unpack the given packed latents """
    ...
    def prepare_latents(
        ...
        """ Generates random noise vectors for diffusion process """
        # VAE applies 8x compression on images but we must also account for packing which requires
        # latent height and width to be divisible by 2.
        height = 2 * (int(height) // (self.vae_scale_factor * 2))
        width = 2 * (int(width) // (self.vae_scale_factor * 2))
    ...
    def guidance_scale(self):
        """ The class property that returns the current `guidance_scale` parameter."""
        return self._guidance_scale

    @property
    def joint_attention_kwargs(self):
        """ The class property that returns the current `joint_attention_kwargs` parameter."""
        return self._joint_attention_kwargs

    @property
    def num_timesteps(self):
        """ The number of inference steps (i.e., denoising steps) to run the pipeline for."""
        return self._num_timesteps

    @property
    def interrupt(self):
        """ The flag indicating whether the pipeline execution should be interrupted """
    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.Tensor]`, `List[PIL.Image.Image]`, 
            or `List[np.ndarray]`):
