...
from .modules.load_util import load_flux_fill_nf4, load_simple_flux_fill_gguf, load_simple_flux_fill_nf4, load_vae
from folder_paths import models_dir, get_filename_list, get_full_path_or_raise, supported_pt_extensions

supported_pt_extensions.add(".gguf")
...
class FluxGuffInpainting:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model_path": (cls.guff_model_list(), ),
                "conditioning": ("CONDITIONING", ),
                "image": ("IMAGE",),
                "mask": ("MASK",),
                "num_inference_steps": ("INT", {"default": 50, "min": 10, "max": 60, "step": 1}),
    ...
    @staticmethod
    def guff_model_list():
        unet_models = get_filename_list("unet")
        return list(filter(lambda x: os.path.basename(x).endswith(".gguf"), unet_models))
    ...
    def inpainting(self,
        ...
        cond_from = cond_from.to(dtype=torch.bfloat16)
        pooled_output_from = pooled_output_from.to(dtype=torch.bfloat16)
        guff_model_path = get_full_path_or_raise("unet", model_path)
        try:
            pipeline = _pipeline
            logging.info("Loading Flux NF4 Inpainting")
            if not cached or pipeline is None:
...
NODE_CLASS_MAPPINGS = {
    ...
    "FluxGuffInpainting": FluxGuffInpainting
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "FluxGuffInpainting": "Flux GUFF Inpainting"
}
